...
const becomeSellerApp = new Vue({
    ...
    methods: {
        ...
        handleContinueClick: function() {
            let rate = this.sharedState.extra.rate ? Math.round(+this.sharedState.extra.rate * 100) : null;
            rate = isNaN(rate) ? this.sharedState.extra.rate : rate;

            let data = {
                description: this.sharedState.extra.description,
                headline: this.sharedState.extra.headline,
                rate: rate,
                ...
                if (res.response.status === 400) {
                    ...
                    if (res.response.data && res.response.data.error && res.response.data.error.fields) {
                        ...
                        this.validationErrorsList = Object.keys(res.response.data.error.fields).map(key => {
                            ...
                            if (key === 'rate') {
                                return 'Incorrect value for hourly rate';
                            }
                            if (key === 'photo') {
