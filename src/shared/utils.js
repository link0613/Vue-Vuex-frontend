...
export function prepareNotification(notification) {
    ...
    switch (notification.type) {
        ...
        case Types.SELLER_ORDER_COMPLETED:
            notification._notification = `${notification.meta.seller.username} has closed order #${notification.meta.order.id} of ${notification.meta.service.title}`;
            notification._url = storeInstance.urlFor('order', [notification.meta.order.id]);
            break;
        ...
        case Types.BUYER_ORDER_OFFER:
            notification._notification = `${notification.meta.seller.username} has offered you some extras for order #${notification.meta.order.id} of ${notification.meta.service.title}`;
            notification._url = storeInstance.urlFor('order', [notification.meta.order.id]);
            break;

        case Types.BUYER_ENQUIRY_OFFER:
            notification._notification = `${notification.meta.seller.username} has made you a private offer on service ${notification.meta.service.title}`;
            notification._url = storeInstance.urlFor('inbox', ['enquiry', notification.meta.enquiry.id]);
            break;
    ...
    if (notification.meta && notification.meta.service) {
        notification._image_url = '/account/service/photo/' + notification.meta.service.id;
    }
...
export function parseHashString() {
    ...
    window.location.hash.slice(2).split('&').forEach(param => {
        let expr = param.split('=');
        if (expr.length !== 2) {
            return;
        }
